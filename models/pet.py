...
class BasePETCount(nn.Module):
    ...
    def forward(self, samples, features, context_info, **kwargs): #samples:输入的图片数据 features:骨干网络提取特征 context_info:编码后的特征、位置编码和掩码
        encode_src, src_pos_embed, mask = context_info #编码后的特征、位置编码和掩码

        # get points queries for transformer
        pqs = self.get_point_query(samples, features, **kwargs) #生成点查询相关的嵌入和坐标
        
        # point querying
        kwargs['pq_stride'] = self.pq_stride #点查询的步幅
        hs = self.transformer(encode_src, src_pos_embed, mask, pqs, img_shape=samples.tensors.shape[-2:], **kwargs)

        # prediction
        points_queries = pqs[1] #取出点坐标
        outputs = self.predict(samples, points_queries, hs, **kwargs) #进行最终的预测
        return outputs
...
class PET(nn.Module):
    ...
    def pet_forward(self, samples, features, pos, **kwargs):
        ...
        sp_h, sp_w = src_h, src_w #稀疏区域使用原始特征图大小
        ds_h, ds_w = int(src_h * 2), int(src_w * 2) #密集区域使用特征图的2倍大小，更细粒度
        split_map = self.quadtree_splitter(encode_src)  #得到分割图，表示每个区域的稀疏/密集概率      
...
class SetCriterion(nn.Module):
    ...
    def _get_src_permutation_idx(self, indices):
        ...
        # print(f'src batch_idx: {batch_idx}, src_idx: {src_idx}')
    ...
    def _get_tgt_permutation_idx(self, indices):
        ...
        # print(f'tgt batch_idx: {batch_idx}, tgt_idx: {tgt_idx}')
