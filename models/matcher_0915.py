...
class HungarianMatcher(nn.Module):
    """
    Hungarian Matcher with additional mask-based constraint.
    """
    def __init__(self, cost_class: float = 1, cost_point: float = 1, cost_mask: float = 1):
        """
        Params:
            cost_class: 权重 - 分类代价
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        outputs: dict {
            "pred_logits": Tensor [bs, num_queries, 2],
            "pred_points": Tensor [bs, num_queries, 2],
            "img_shape": (H, W)
        }
        targets: list[dict] * bs, each dict {
            "labels": Tensor [num_gt],
            "points": Tensor [num_gt, 2],
            "masks": Tensor [num_gt, H, W]   # 0/1 掩码
        }
        """
        ...
        # tgt_keys = torch.cat([v["keys"] for v in targets])             # [sum_gt]
        # tgt_masks = torch.cat([v["masks"] for v in targets])         # [sum_gt, H, W]
        # 生成所有 GT 的掩码列表
        masks_list = []
        for i in range(bs):
        ...
        # print(f' tgt_masks shape: {tgt_masks.shape}, tgt_points shape: {tgt_points.shape}')
        # tgt_masks = torch.cat([
        #     v["masks"].unsqueeze(0) if v["masks"].ndim == 2 else v["masks"]
        #     for v in targets
        # ]) 
        ...
        # 遍历每个预测点，判断是否落在对应的 GT mask 内
        # out_points_abs 是 [bs*num_queries, 2]
        for j, tgt_mask in enumerate(tgt_masks):  # 遍历所有 GT mask
            # 生成每个预测点的坐标
            xs = out_points_abs[:, 0].long().clamp(0, img_h - 1)
            ys = out_points_abs[:, 1].long().clamp(0, img_w - 1)

            # 查 mask 上的值
            inside_mask = tgt_mask[xs, ys]  # [bs*num_queries], 0/1
            penalty = (1 - inside_mask.float())  # 在 mask 内 → 0，mask 外 → 1
            cost_mask[:, j] = penalty * 1.0  # 可以调节 penalty 值

        # 最终 cost
        # print(f'out_prob shape:{out_prob.shape}, tgt_ids shape:{tgt_ids.shape}, tgt_points shape:{tgt_points.shape}, tgt_masks shape:{tgt_masks.shape}')
        # print(f'cost_point shape: {cost_point.shape}, cost_class shape: {cost_class.shape}, cost_mask shape: {cost_mask.shape}')
