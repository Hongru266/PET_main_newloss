...
class UCF(Dataset):
    def __init__(self, data_root, transform=None, train=False, flip=False):
        ...
        self.prefix = prefix
        self.img_list = [os.path.basename(p) for p in glob.glob(f"{data_root}/{prefix}/*.jpg")]
        ...
        for img_name in self.img_list:
            img_path = f"{data_root}/{prefix}/{img_name}"  
            gt_path = f"{data_root}/{prefix}/{img_name}"
            self.gt_list[img_path] = gt_path.replace(".jpg", "_ann.mat")
            if self.train:
                img_name = img_name.upper()
                masks_path = f"{data_root}/masks/{img_name}"
                dict_path = f"{data_root}/dict/{img_name}"
    ...
    def __getitem__(self, index):
        ...
        points_max = points[:, 0].max() if points.shape[0] > 0 else "N/A"
        # print(f'after random crop: img shape: {img.shape}, points shape: {points.shape}, points[:, 0] max:{points_max}, masks shape: {masks.shape if len(masks) > 0 else "N/A"}, dicts length: {len(dicts)}')
        ...
        # target['labels'] = torch.ones(target['dicts_coords'].shape[0] if 'dicts_coords' in target else points.shape[0]).long()
        ...
        # print(f'target labels shape:{target["labels"].shape}, unique labels length:{len(unique_labels)}, target masks shape:{target["masks"].shape}, target dicts_keys shape:{len(target["dicts_keys"])}, target dicts_coords shape:{target["dicts_coords"].shape}')
...
def build(image_set, args):
    ...
    if image_set == 'train':
        train_set = UCF(data_root, train=True, transform=transform, flip=True)
        return train_set
    elif image_set == 'val':
        val_set = UCF(data_root, train=False, transform=transform)
        return val_set